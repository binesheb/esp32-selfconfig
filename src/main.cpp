...
bool findRelease(String &releaseJson) {
  ...
  if (config.channel == "stable") {
    return httpsGetString(base + "/releases/latest", releaseJson);
  }
...
bool downloadAndInstall(const String &firmwareUrl, size_t expectedSize, const String &expectedSha) {
  ...
  if (expectedSize > 0 && total > 0 && expectedSize != (size_t)total) {
    Serial.printf("[OTA] Size mismatch: manifest=%u download=%d\n", (unsigned)expectedSize, total);
    http.end();
    return false;
  }
  ...
  if (expectedSha.length() != 64 || actualSha != expectedSha) {
...
bool checkForUpdate() {
  ...
  String releaseJson;
  if (!findRelease(releaseJson)) {
    Serial.println("[OTA] No usable GitHub release found");
    return false;
  }

  JsonDocument releaseDoc;
  if (deserializeJson(releaseDoc, releaseJson)) {
    Serial.println("[OTA] Invalid release JSON");
    return false;
  }
  ...
  if (manifestUrl.isEmpty()) {
    Serial.println("[OTA] Release has no manifest.json asset");
    return false;
  }
  ...
  if (firmwareUrl.isEmpty()) {
    firmwareUrl = "https://github.com/" + config.githubOwner + "/" + config.githubRepo + "/releases/download/" + tag + "/firmware.bin";
  }
  ...
  if (sha.length() != 64) {
    Serial.println("[OTA] Invalid SHA-256 in manifest");
    return false;
  }

  if (downloadAndInstall(firmwareUrl, firmwareSize, sha)) {
    delay(1000);
    ESP.restart();
    return true;
  }
...
void checkSetupButton() {
  ...
  if (held >= FACTORY_RESET_HOLD_MS) {
    factoryReset();
  } else if (held >= SETUP_HOLD_MS && !setupMode) {
    startSetupPortal();
  }
}
...
void setup() {
  ...
  if (!isConfigured() || !connectWiFi()) {
    ...
    if (config.autoUpdate) checkForUpdate();
    applicationSetup();
  }
}
